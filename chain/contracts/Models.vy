...
NAME: constant(String[15]) = "Global Models"
...
def __init__(trainers: address):
    ...
    self.totalSupply = 1
    self.hashToId[empty(bytes32)] = self.totalSupply
    self.idWeight[self.totalSupply] += 1
...
def _append(parent: bytes32, child: bytes32, URI: String[128]) -> bytes32:
    ...
    hash: bytes32 = keccak256(_abi_encode(parent, child))
    id: uint256 = self.totalSupply
    if self._hashExists(hash):
        id = self.hashToId[hash]
...
@internal
@view
def _hashExists(hash: bytes32) -> bool:
    return self.hashToId[hash] > 0

@view
@external
def modelExists(hash: bytes32) -> bool:
    return self._hashExists(hash)

@external
def getBytes() -> bytes32:
    return convert(10, bytes32)
