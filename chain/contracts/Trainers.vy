...
ownerToId: public(HashMap[address, uint256])
...
NAME: constant(String[20]) = "trainer"
SYMBOL: constant(String[5]) = "TRAIN"
...
def register() -> bool:
    ...
    """ 
    assert not self._isRegistered(msg.sender)
    self.totalSupply += 1
    assert self.idToOwner[self.totalSupply] == empty(address)  # Sanity check
    
    self.idToOwner[self.totalSupply] = msg.sender
    self.ownerToId[msg.sender] = self.totalSupply
    log TrainerRegistered(msg.sender, self.totalSupply)
...
@view
@internal
def _isRegistered(trainer: address) -> bool:
    return self.ownerToId[trainer] > 0
...
def _transferFrom(owner: address, receiver: address, tokenId: uint256, sender: address):
    ...
    assert not self._isRegistered(receiver)
    ...
    self.ownerToId[owner] = 0
    self.ownerToId[receiver] = tokenId
    # Log the transfer
